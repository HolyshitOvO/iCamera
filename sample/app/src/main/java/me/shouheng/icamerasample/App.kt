...
//        LeakCanary.install(this)
        ConfigurationProvider.get().addLogWatcher(object : LogWatcher {
            override fun onLog(level: LogLevel, tag: String, msg: String?) {
                when(level) {
                    LogLevel.V -> { Log.v("__I_CAMERA_$tag", msg.toString()) }
                    LogLevel.I -> { Log.i("__I_CAMERA_$tag", msg.toString()) }
                    LogLevel.D -> { Log.d("__I_CAMERA_$tag", msg.toString())  }
                    LogLevel.W -> { Log.w("__I_CAMERA_$tag", msg.toString())  }
                    LogLevel.E -> { Log.e("__I_CAMERA_$tag", msg.toString())  }
                }
            }
        })
    }
